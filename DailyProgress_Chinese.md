...
- 为 Random Mode 新增难度选择步骤，通过简单、中等和困难配置建筑数量、车辆生成间隔、模拟运行时间与道路预算。
- 新增 `UCityFlowDifficultyWidget`，支持蓝图控件绑定、悬停切换的一块本地化参数详情、返回流程，并在未配置 Widget 蓝图时提供完整原生回退界面。
- 保留无参数 `StartRandomPlanningGame()` 作为默认中等难度的兼容入口，并新增显式难度感知的 GameMode API。
- 将车辆生成重构为目标在途数量、批量补充脉冲、逐次起点阻挡检查、路线回退和硬数量上限机制。
- 让模拟倒计时和运行时计分输入使用每局活动的模拟时长与车辆生成间隔。
...
- 使用最终地基轮廓的多级内偏移重新生成人行道，使其保留圆角和逐边连接形状。
- 添加了可配置的向内偏移、内外几何倒角、接缝嵌入深度、圆角细分数，以及独立的地基/人行道碰撞烘焙开关。
- 添加了轮廓清洗、有限斜接交点、基于 footprint 的参数限制和无效多边形拒绝机制，防止生成退化或折叠网格。
- 更新了中英文 GDD 与 TDD，记录改进后的地基视觉规范、蓝图属性和网格生成规则。
- 完成了 foundation 与 sidewalk 改动的 `CityFlowEditor Win64 Development` 编译验证。

## 2026-06-19

### 主菜单、教程、设置与音频

- 移除 Start Game 流程，新增 Tutorial 和 Settings 入口，同时保留 Random Mode 与标题界面动态预览。
- 通过 `UCityFlowTutorialDataAsset`、可本地化教程条目、可选的条目图片和 `UCityFlowTutorialWidget` 蓝图扩展事件，加入数据驱动教程支持。
- 通过 `UCityFlowSettingsWidget` 和 SaveGame 持久化主音量、SFX 音量与语言设置，并使用 Unreal 原生 Culture 切换。
- 加入由 HUD 管理的背景音乐和 SoundClass 覆写支持；各音频资产仍需正确路由到 SoundClass，才能被全局 SFX 滑块统一控制。

### 连通性安全的 L-System 重构

- 将 `ULSystemManager` 重构为混合生成器：先预留门口到主道路连通分量的连接路径，再用剩余预算生成有机分支。
- 用真正的同一连通分量验证替代旧的可达性假设，并支持复用现有道路、多源门口路径搜索和精确的连接预算预留。
- 加入旋转后门口方向处理、拓扑安全的死胡同/直路段分支候选、去重优先级生长、全局门口吸引、确定性的 `FRandomStream` 和严格分支预算上限。
- 保持现有公开控制接口和委托兼容；玩家触发时遵守 `LSystemBudgetShare`，标题自动预览则可使用保障连通所需的全部剩余预算。
- 通过运行时冒烟测试验证新生成器能够让全部建筑接入同一道路分量并正常结束生成。

### 本地化与运行时稳定性

- 将 Source 中面向玩家的原生文本改为 `LOCTEXT` 或 `NSLOCTEXT`，动态数值使用具备 Culture 感知能力的 `FText` 格式化生成。
- 验证 Source 本地化收集结果为 49 个条目、无命名空间/键冲突，并完成 Editor 构建。
- 将车辆死亡委托处理函数暴露为 `UFUNCTION`，修复动态委托绑定。
- 加固 GridManager 与 ScoringManager 的退出流程，避免子系统销毁末期访问空网格，或在 Grid 已销毁后再次计算最终分数。
- 通过自然退出冒烟测试确认此前的数组越界断言不再出现。

### 文档

- 更新中英文 GDD，补充辅助毛细道路规则，以及新版主菜单、教程、设置、音频和本地化体验。
- 更新中英文 TDD，记录 L-System 连接规划器、有机生长机制、菜单/Widget 架构、音频持久化与路由、原生文本本地化和退出安全机制。
