...
`ABuilding` 继承自 `AMeshGridPlaceableActor`，提供多单元格建筑放置及可配置出入口功能。
...
| `Doorways` (`TArray<FBuildingDoorway>`) | 手动配置的出入口列表 |
...
- **视觉居中：** 放置后（`OnPlacedOnGrid`），Actor 从左上角网格锚点重定位到覆盖区域的几何中心，并将 Mesh 缩放至填充整个占地区域：`Scale = BuildingSize × CellSize / ReferenceMeshSize`。
- **预览：** `UpdatePreviewAppearance()` 实时居中并缩放预览 Actor，让玩家在放置前看到建筑占地区域。
...
{
    FGridVector RelativePosition;   // 相对于建筑左上角网格锚点的坐标
    EGridDirection FacingDirection; // 出入口朝向
};
```

- 出入口以网格相对坐标定义（如 `(-1, 0)` 表示建筑左侧，朝向右）。
- `AutoGenerateDoorways()` 计算各边中点：对 `W×H` 建筑，在 `(-1, Floor((H-1)/2))`（左）、`(W, Floor((H-1)/2))`（右）、`(Floor((W-1)/2), -1)`（上）、`(Floor((W-1)/2), H)`（下）生成，各朝向建筑内部。
- `GetDoorwayWorldPositions()` 将相对坐标转换为绝对网格坐标。
- 出入口完全可在蓝图中配置 — 可添加、删除或重新定位。

#### GridManager 随机放置（调试）

`UGridManager::TryPlaceBuildingRandom(TSubclassOf<AGridPlaceableActor>)`

- 扫描网格找出建筑占地区域（基于 CDO 的 `BuildingSize`）完全落在空单元格上的所有合法位置。
- 若存在合法位置，随机选择一个，生成 Actor 并调用 `PlaceOnGrid()`。
- 若无合法位置则返回 `nullptr`。
- 可在关卡蓝图中调用，便于调试时快速填充地图。

#### 连接检测（规划中）

- 每个出入口记录其所属建筑及连接状态。
- 当道路单元格恰好放置在出入口位置时，建筑自动标记为**已连接**。
