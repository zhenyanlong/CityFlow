...
`UVehicleManager` 现在采用**目标在途数量补充机制**，不再假设每个间隔只生成一辆车就足以维持道路密度：

1. GameMode 在 `StartSpawning()` 前通过 `ConfigureSpawnProfile(Interval, Target, Max, Burst)` 传入本局参数。
2. 每个 `SpawnInterval` 计算可用空缺：`min(Target - ActiveCount, Max - ActiveCount)`。
3. 单次脉冲最多执行 `SpawnBurstSize` 次成功生成。
4. 每次批量尝试都会重新收集起点并调用 `IsBuildingBlocked()`，避免多辆车堆叠在同一建筑门口。
5. 每次尝试打乱终点顺序；首组起终点不存在有效 A\* 路线时，会继续尝试其他起点。
6. 达到目标在途数量后自动暂停补充；车辆抵达或死亡产生空缺后恢复生成，`MaxActiveVehicleCount` 始终作为硬上限。

非难度对局使用 `UCityFlowDeveloperSettings` 的 `VehicleSpawnInterval`、`TargetActiveVehicleCount`、`MaxSpawnBurstSize` 和 `MaxVehicleCount`；Random Mode 难度配置会覆盖这四项运行时参数。
...
| 阶段 | 转换 | 动作 |
|---|---|---|
| **None** → **Planning** | `StartNewGame()` 或 `StartRandomPlanningGameWithDifficulty()` | 初始化网格、生成建筑并设置预算；难度对局随机化 seed/网格尺寸，并应用选中的建筑、预算、时长和车流配置 |
...
`StartRandomPlanningGameWithDifficulty(ECityFlowDifficulty)` 是玩家进入 Random Mode 的流程。它保留随机种子和网格尺寸 helper，再用选中的 `FCityFlowDifficultyProfile` 覆盖建筑数量与道路预算。系统只生成景观和建筑并切换到 Planning，道路铺设、L-system 和模拟启动仍由玩家控制。原有无参数 `StartRandomPlanningGame()` 保持蓝图兼容，并默认选择中等难度。

#### 难度配置与本局活动参数

GameMode 蓝图可编辑 `EasyDifficultyProfile`、`MediumDifficultyProfile` 和 `HardDifficultyProfile`。每套配置包含四项玩家可见参数，以及内部车流密度控制参数：

| 配置 | 建筑 | 生成间隔 | 运行时间 | 预算 | 目标在途 | 单次批量 | 硬上限 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 简单 | 8 | 0.90 秒 | 120 秒 | 220 | 16 | 2 | 24 |
| 中等 | 12 | 0.65 秒 | 180 秒 | 230 | 26 | 3 | 36 |
| 困难 | 16 | 0.45 秒 | 240 秒 | 240 | 38 | 4 | 50 |

`ApplyDifficultyProfile()` 将选择复制到本局活动字段，不修改项目默认配置。模拟计时器和 HUD 倒计时读取 `GetActiveSimulationDuration()`；Planning → Simulating 时 VehicleManager 接收当前车流配置；最终计分中的运行需求使用本局时长与生成间隔。标题自动预览调用 `ResetActiveMatchSettings()`，继续使用 DeveloperSettings/GameMode 默认值。
...
| `StartRandomPlanningGameWithDifficulty(ECityFlowDifficulty)` | 应用选中的难度配置，再创建玩家可操作的随机 Planning 开局。 |
| `GetDifficultyProfile(ECityFlowDifficulty)` | 返回难度界面和本局初始化使用的蓝图可配置参数。 |
...
EvaluationWidget (结算)
  ...
  └─ Btn_Restart → HUD::HandleRestartClicked() → ShowGameWidgetRandom(CurrentDifficulty) → 随机 Planning 开局
```
...
- `ShowDifficultyWidget()` 保持菜单背景预览并显示难度选择；`ShowGameWidgetRandom(Difficulty)` 通过 `StartRandomPlanningGameWithDifficulty()` 启动随机 Planning 对局，关闭标题摄像机旋转并开启放置。
...
| `DifficultyWidgetClass` | `TSubclassOf<UCityFlowDifficultyWidget>` | 可选的 Random Mode 自定义难度界面；未配置时使用原生回退 UI |
...
- `Btn_Restart` → 广播 `OnRestartClicked`（HUD → `HandleRestartClicked` → `ShowGameWidgetRandom(CurrentDifficulty)` → 随机 Planning 开局）
...
- `Btn_RandomMode` → 广播 `OnRandomModeClicked`（HUD → `ShowDifficultyWidget()`；选择难度后调用 `ShowGameWidgetRandom(Difficulty)` 并开启放置）
...
- `UCityFlowDifficultyWidget` 提供可选绑定：`Btn_Easy`、`Btn_Medium`、`Btn_Hard`、`Btn_Back` 和一块共享的 `Txt_DifficultyDetails`。
- 共享详情默认显示中等难度；任一难度按钮获得鼠标悬停焦点时，切换为对应配置内容。
- HUD 未配置 `DifficultyWidgetClass` 时会创建原生类，由 `BuildFallbackLayout()` 在运行时构建完整且可本地化的选择界面。
- 自定义 `WBP_DifficultyWidget` 可继承原生类并绑定同名控件；参数详情从 GameMode 自动填充，保证蓝图显示与实际运行值一致。
- `OnDifficultySelected(ECityFlowDifficulty)` 将选择返回 HUD；Back 回到 StartWidget，且不会终止正在运行的菜单背景预览。
- Evaluation 的 Restart 沿用当前局难度；从主菜单重新进入 Random Mode 时再次显示选择界面。
...
- 原生条目生成提供可配置的普通/选中 `FButtonStyle`、文字字体与颜色、内容边距和列表槽位边距。系统保留生成的控件引用，使 `SelectTutorial()` 能持续应用选中样式；运行时修改外观后可调用 `RefreshGeneratedEntryStyles()` 重新应用。
