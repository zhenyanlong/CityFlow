...
- **Z-axis convention:** Foundation sits on ground (Z=0), extruding upward to `FoundationHeight`. Sidewalk rises to `FoundationHeight + SidewalkHeight` and extends down by `SidewalkEmbedDepth` into the foundation to hide contact seams.
...
- **Outline-derived sidewalk:** The sidewalk is generated by successively insetting the sanitized final foundation outline, so it preserves rounded corners and per-edge road-connection changes instead of reverting to a rectangular footprint.
...
| `CornerSegments` | 8 | Samples per rounded corner (clamped to 1–32) |
| `SidewalkWidth` | 20 | Sidewalk ring width measured inward from its outer edge |
| `SidewalkInset` | 10 | Inward offset from the final foundation outline to the sidewalk outer edge |
| `SidewalkHeight` | 10 | Sidewalk extrusion height above foundation top |
| `SidewalkBevelSize` | 4 | Horizontal bevel depth on both outer and inner top edges |
| `SidewalkBevelHeight` | 4 | Vertical height of the sidewalk bevel |
| `SidewalkEmbedDepth` | 1 | Depth embedded into the foundation top to suppress seams |
| `bCreateFoundationCollision` | `true` | Enables procedural collision cooking for the foundation mesh |
| `bCreateSidewalkCollision` | `true` | Enables procedural collision cooking for the sidewalk mesh |
...
| **Sidewalk outline rings** | Four equal-vertex inset outlines: outer/inner wall bases plus outer/inner top edges | Sanitized CCW outlines |
| **Sidewalk outer/inner walls** | One quad per foundation outline segment, including rounded-corner segments | Outward-facing CW |
| **Sidewalk bevels** | Sloped quads connect both wall rings to the narrowed top ring with diagonal normals | Outward-facing CW |
| **Sidewalk top/bottom** | Segment quads close the ring at `TopZ`; bottom is embedded by `SidewalkEmbedDepth` | Upward/downward CW |

Inset values and bevel dimensions are clamped against the current footprint size and sidewalk width. Offset-line intersections use a bounded miter fallback, and invalid or inverted inset polygons abort sidewalk generation instead of emitting folded geometry. Duplicate adjacent outline samples are removed before either mesh is generated.
