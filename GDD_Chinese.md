...
建筑地基采用克制的圆角，人行道环略微内收于地基轮廓，并在内外边缘使用小幅倒角。这样可以避免薄片式悬挑和明显的黑色接触缝，同时从游戏镜头下仍能清楚辨认建筑 footprint 及门口与道路的关系。

### 4.1 建筑展示关卡

单独的非玩法展示关卡会在 Random Mode 使用的同一套真实网格上呈现所有已配置建筑。建筑以便于观察的间距自动排列，并可选择展示四个旋转方向，用于集中检查占地缩放、地基、门口朝向、生成动画和整体美术一致性。展示关卡不进入主菜单、计分、交通或自动对局状态机。
